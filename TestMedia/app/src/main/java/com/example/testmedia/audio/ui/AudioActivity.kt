...
import com.example.testmedia.audio.viewmodel.MusicViewModel
import com.example.testmedia.audio.adapters.QueueAdapter
import com.example.testmedia.audio.extensions.*
import com.example.testmedia.audio.goPreferences
import com.example.testmedia.audio.helpers.*
import com.example.testmedia.audio.models.Album
import com.example.testmedia.audio.models.Music
import com.example.testmedia.audio.player.EqualizerUtils
import com.example.testmedia.audio.player.MediaPlayerHolder
import com.example.testmedia.audio.player.MediaPlayerInterface
import com.example.testmedia.audio.player.PlayerService
import com.example.testmedia.audio.ui.fragments.*
import com.example.testmedia.databinding.*
...
class AudioActivity : AppCompatActivity(), UIControlInterface {

    // View binding classes
    private lateinit var mAudioActivityBinding: AudioActivityBinding
    ...
    private val connection = object : ServiceConnection {
        override fun onServiceConnected(componentName: ComponentName, service: IBinder) {
            ...
            mMusicViewModel.deviceMusic.observe(this@AudioActivity, { returnedMusic ->
    ...
    override fun onBackPressed() {
        when {
            ...
            else -> if (mAudioActivityBinding.viewPager2.currentItem != 0) {
                mAudioActivityBinding.viewPager2.currentItem =
                    0
            } else {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mAudioActivityBinding = AudioActivityBinding.inflate(layoutInflater)
        mPlayerControlsPanelBinding = PlayerControlsPanelBinding.bind(mAudioActivityBinding.root)

        setContentView(mAudioActivityBinding.root)

        if (VersioningHelper.isOreoMR1()) {
            edgeToEdge {
                mAudioActivityBinding.root.fit { Edge.Top + Edge.Bottom }
            }
        }
    ...
    private fun notifyError(errorType: String) {
        ...
        mAudioActivityBinding.loadingProgressBar.handleViewVisibility(false)
        mAudioActivityBinding.viewPager2.handleViewVisibility(false)
        if (sAllowCommit) {
    ...
    private fun finishSetup(music: MutableList<Music>?) {
        ...
        if (!music.isNullOrEmpty()) {
            ...
            mAudioActivityBinding.loadingProgressBar.animate().run {
                duration = 750
                alpha(0.0F)
                withStartAction {
    ...
    private fun initViewPager() {
        ...
        mAudioActivityBinding.viewPager2.offscreenPageLimit = mActiveFragments.size.minus(1)
        mAudioActivityBinding.viewPager2.adapter = pagerAdapter

        if (mFragmentToRestore != 0) {
            mAudioActivityBinding.viewPager2.setCurrentItem(
                mFragmentToRestore,
                false
            )
        }
    ...
    private fun initTabLayout() {
        ...
        mPlayerControlsPanelBinding.tabLayout.run {
            ...
            TabLayoutMediator(this, mAudioActivityBinding.viewPager2) { tab, position ->
                tab.setIcon(ThemeHelper.getTabIcon(mActiveFragments[position]))
            }.attach()

            addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
                override fun onTabSelected(tab: TabLayout.Tab) {
                    tab.icon?.setTint(ThemeHelper.resolveThemeAccent(this@AudioActivity))
                }
            ...
            getTabAt(mFragmentToRestore)?.icon?.setTint(
                ThemeHelper.resolveThemeAccent(this@AudioActivity)
            )
    ...
    private fun setupSeekBarProgressListener() {
        ...
        mNpBinding.npSeekBar.setOnSeekBarChangeListener(
            object : SeekBar.OnSeekBarChangeListener {
                ...
                val selectedColor = ThemeHelper.resolveThemeAccent(this@AudioActivity)
        ...
        mNpCoverBinding.run {
            ...
            if (VersioningHelper.isMarshmallow()) {
                ...
                npPlaybackSpeed.setOnClickListener { view ->
                    DialogHelper.showPopupForPlaybackSpeed(this@AudioActivity, view)
                }
            } else {
            ...
            npLove.setOnClickListener {
                ...
                onLovedSongsUpdate(false)
                updateNpFavoritesIcon(this@AudioActivity)
            }
    ...
    private fun setupNPCoverButtonsToasts(vararg imageButtons: ImageButton) {
        ...
        while (iterator.hasNext()) {
            iterator.next().setOnLongClickListener { btn ->
                Toast.makeText(this@AudioActivity, btn.contentDescription, Toast.LENGTH_SHORT).show()
                return@setOnLongClickListener true
            }
        }
    }
                ...
                mNpControlsBinding.npRepeat.run {
                    ...
                    ThemeHelper.updateIconTint(
                        ...
                        if (mMediaPlayerHolder.isRepeat1X || mMediaPlayerHolder.isLooping) {
                            ...
                            ContextCompat.getColor(this@AudioActivity, R.color.widgetsColor)
                        }
                    )
    ...
    override fun onAppearanceChanged(isThemeChanged: Boolean) {
        mFragmentToRestore = mAudioActivityBinding.viewPager2.currentItem
        ...
        if (isMediaPlayerHolder) {
            ...
            mMediaPlayerHolder.run {
                ...
                onUpdateDefaultAlbumArt(
                    ContextCompat.getDrawable(this@AudioActivity, R.drawable.album_art)?.toBitmap()
                )
            }
        }
    }
    ...
    override fun onSongVisualizationChanged() {
        ...
        if (mAllMusicFragment != null && !mAllMusicFragment?.onSongVisualizationChanged()!!) {
            ThemeHelper.applyChanges(this, mAudioActivityBinding.viewPager2.currentItem)
        }
    }
    ...
    private val mMediaPlayerInterface = object : MediaPlayerInterface {
        ...
        override fun onQueueStartedOrEnded(started: Boolean) {
            ThemeHelper.updateIconTint(
                mPlayerControlsPanelBinding.queueButton,
                when {
                    started -> ThemeHelper.resolveThemeAccent(this@AudioActivity)
        ...
        override fun onPlaylistEnded() {
            Toast.makeText(this@AudioActivity, getString(R.string.error_list_ended), Toast.LENGTH_LONG)
                .show()
        }
    }
